...
class XFeat(nn.Module):
	...
	def __init__(self, weights = os.path.abspath(os.path.dirname(__file__)) + '/../weights/xfeat_default_160000.pth', top_k = 4096, detection_threshold=0.05):
		super().__init__()
	...
	def time_cal(self, dot_time, name):
		if name in self.timelistname:	
			# index = self.timelistname.index(name)
			# print(name,self.timelist[index],"add: ",time.time() - dot_time)
			self.timelist[self.timelistname.index(name)]+=(time.time() - dot_time)
		else:
			self.timelist.append(time.time() - dot_time)
			self.timelistname.append(name)
	...
	def match_xfeat_star(self, im_set1, im_set2, top_k = None):
		...
		self.time_cal(dot_dAC,"detectAndComputeDense")

		#Match batches of pairs
		dot_batchmatch = time.time()
		idxs_list = self.batch_match(out1['descriptors'], out2['descriptors'])
		B = len(im_set1)
		self.time_cal(dot_batchmatch,"batch_match")

		#Refine coarse matches
		#this part is harder to batch, currently iterate
		dot_refinematch = time.time()
		matches = []
		for b in range(B):
			matches.append(self.refine_matches(out1, out2, matches = idxs_list, batch_idx=b))

		self.time_cal(dot_refinematch,"refine_matches")
		self.time_cal(dot_start,"total_time")
		return matches if B > 1 else (matches[0][:, :2].cpu().numpy(), matches[0][:, 2:].cpu().numpy(),self.timelist ,self.timelistname)
	...
	def batch_match(self, feats1, feats2, min_cossim = -1):
		batch_match_pre_dot = time.time()
		B = len(feats1)

		bmm_dot = time.time()
		cossim = torch.bmm(feats1, feats2.permute(0,2,1))
		self.time_cal(bmm_dot,"bmm")

		argmax_dot = time.time()
		match12 = torch.argmax(cossim, dim=-1)
		match21 = torch.argmax(cossim.permute(0,2,1), dim=-1)
		self.time_cal(argmax_dot,"argmax")

		idx0 = torch.arange(len(match12[0]), device=match12.device)
		self.time_cal(batch_match_pre_dot,"batch_match_pre")

		batched_matches = []

		batch_match_main_dot = time.time()
		for b in range(B):
		...
		self.time_cal(batch_match_main_dot,"batch_match_main")
	...
	def extractDense(self, x, top_k = 8_000):
		...
		dot_preprocess_tensor = time.time()
		x, rh1, rw1 = self.preprocess_tensor(x)
		self.time_cal(dot_preprocess_tensor,"preprocess_tensor")

		dot_net = time.time()
		M1, K1, H1 = self.net(x)
		self.time_cal(dot_net,"net")
	...
	def extract_dualscale(self, x, top_k, s1 = 0.6, s2 = 1.3):
		...
		self.time_cal(dot_interpolate,"interpolate")

		B, _, _, _ = x.shape

		dot_extractDense = time.time()
		mkpts_1, feats_1 = self.extractDense(x1, int(top_k*0.20))
		mkpts_2, feats_2 = self.extractDense(x2, int(top_k*0.80))
		self.time_cal(dot_extractDense,"extractDense")

		dot_dulextract_cat = time.time()
		...
		self.time_cal(dot_dulextract_cat,"dulextract_cat")
