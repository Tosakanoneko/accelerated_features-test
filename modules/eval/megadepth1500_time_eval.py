...
def run_pose_benchmark(matcher_fn, loader, ransac_thr=2.5):
    ...
    for d in tqdm.tqdm(loader):
        src_pts, dst_pts, timelist, timelist_name = matcher_fn(tensor2bgr(d['image0']), tensor2bgr(d['image1']))
        ...
        if cnt==0:
            for i in range(0, len(timelist)):
                #print(timelist_name[i],":",timelist[i]/total*100)
                total_timelist.append(timelist[i]/total*100)
        else:
            for i in range(0, len(timelist)):
                #print(timelist_name[i],":",timelist[i]/total*100)
                total_timelist[i] += timelist[i]/total*100
    ...
    compute_maa(pairs)
    for i in range(0, len(total_timelist)):
        print("\n",timelist_name[i],"avg time: ",total_timelist[i]/cnt)
    
    print("运行时间：",time.time()-start)   

def parse_args():
    parser = argparse.ArgumentParser(description="Run pose benchmark with matcher")
    parser.add_argument('--dataset-dir', type=str, default="D:/SCIResearch/code/accelerated_features-main/modules/dataset",
                        help="Path to MegaDepth dataset root")
    parser.add_argument('--matcher', type=str, choices=['xfeat', 'xfeat-star', 'alike'], default='xfeat-star',
...
if __name__ == '__main__':
    ...
    dataset = MegaDepth1500( json_file = 'D:/SCIResearch/code/accelerated_features-main/assets/megadepth_1500.json',
                             root_dir =  args.dataset_dir + "/megadepth-1500/Mega1500/megadepth_test_1500")

    loader = DataLoader(dataset, batch_size=1, shuffle=False)

    if args.matcher == 'xfeat':
        print("Running benchmark for XFeat..")
        from modules.xfeat_count import XFeat
        xfeat = XFeat()
